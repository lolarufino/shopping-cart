...
const mutations = {
  ...
  incrementProductAmount(state: State, payload: Product) {
    state.productsInCart.filter((product: any) =>
      product === payload && payload.amount < product.stock ? product.amount++ : null
    );
  },
  decrementProductAmount(state: State, payload: Product) {
    state.productsInCart.filter((product: any) =>
      product === payload && payload.amount >= 1 ? product.amount-- : null
    );
  },
